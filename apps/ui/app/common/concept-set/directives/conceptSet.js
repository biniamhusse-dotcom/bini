...
angular.module('bahmni.common.conceptSet')
    .directive('conceptSet', ['contextChangeHandler', 'appService', '$http', 'observationsService', 'messagingService', 'conceptSetService', 'conceptSetUiConfigService', 'spinner',
        function (contextChangeHandler, appService, $http, observationsService, messagingService, conceptSetService, conceptSetUiConfigService, spinner) {
            var controller = function ($scope) {
                ...
                var multiSelectableConcepts = function () {
                    var params = {
                        q: "emrapi.sqlSearch.multiSelectableConcepts"
                    };
                ...
                var autocompleteConcepts = function () {
                    var params = {
                        q: "emrapi.sqlSearch.autocompleteConcepts"
                    };
                    return $http.get('/openmrs/ws/rest/v1/bahmnicore/sql', {
                        method: "GET",
                        params: params,
                ...
                var dropdownConcepts = function () {
                    var params = {
                        q: "emrapi.sqlSearch.dropdownConcepts"
                    };
                    return $http.get('/openmrs/ws/rest/v1/bahmnicore/sql', {
                        method: "GET",
                        params: params,
                ...
                var requiredConcepts = function () {
                    var params = {
                        q: "emrapi.sqlSearch.requiredConcepts"
                    };
                    return $http.get('/openmrs/ws/rest/v1/bahmnicore/sql', {
                        method: "GET",
                        params: params,
                ...
                var init = function () {
                    return conceptSetService.getConcept({
                        ...
                        multiSelectableConcepts().then(function (multiSelectableResponse) {
                            autocompleteConcepts().then(function (autocompleteResponse) {
                                dropdownConcepts().then(function (dropdownResponse) {
                                    requiredConcepts().then(function (requiredResponse) {
                                        var mergedData = [];

                                        var dropdownData = dropdownResponse.data
                                        var requiredData = requiredResponse.data
                                        var autocompleteData = autocompleteResponse.data
                                        var multiSelectData = multiSelectableResponse.data;

                                        // Merged concepts data
                                        var mergedData = [...autocompleteData, ...multiSelectData, ...dropdownData, ...requiredData];

                                        var resultObject = {};

                                        for (var i = 0; i < mergedData.length; i++) {
                                            var concept = mergedData[i];
                                            if (!resultObject[concept.name]) {
                                                resultObject[concept.name] = {};
                                            }
                                            if (concept.value_reference === 'true') {
                                                resultObject[concept.name][concept.attribute_name] = true;
                                            }
                                        }

                                        conceptSetUIConfig = { ...resultObject, ...conceptSetUIConfig };
