    ...
    .factory('drugService', ['$http', function ($http) {
        ...
        var search = function (drugName, conceptUuid) {
            ...
            return $http.get(Bahmni.Common.Constants.drugUrl, {
                ...
                var matchedItems = response.data.results;
                var stockStatus = JSON.parse(localStorage.getItem('stockStatus'));
                if (stockStatus) {
                    matchedItems.forEach(element => {
                        stockStatus.model.forEach(item => {
                            if (element.uuid === item.itemUnit && item.hasStock === false) {
