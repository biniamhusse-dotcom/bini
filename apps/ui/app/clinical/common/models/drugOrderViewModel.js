...
Bahmni.Clinical.DrugOrderViewModel.createFromContract = function (drugOrderResponse, config) {
    ...
    var getPrescriptionDtpCase = async function (order_number) {
        var params = new URLSearchParams({
            q: "emrapi.getPrescriptionDtpCase",
    ...
    let promise = getPrescriptionDtpCase(drugOrderResponse.orderNumber).then(function (data) {
        if (data.length > 0) {
            data.forEach(dtp => {
                if (viewModel.dtpCases) {
                    viewModel.dtpCases = dtp.value + ", " + viewModel.dtpCases;
                } else {
                    viewModel.dtpCases = dtp.value;
                }
            })
        }
    });
