...
angular.module('bahmni.clinical')
    .controller('ConceptSetPageController', ['$scope', '$rootScope', '$stateParams', 'conceptSetService',
        ...
        'contextChangeHandler', '$q', '$translate', 'formService', 'locationService',
        function ($scope, $rootScope, $stateParams, conceptSetService,
            ...
            var loggedInLocationName;

            $scope.logedinLocation = locationService.getLoggedInLocation().then(function (response) {
                loggedInLocationName = response.name;
                loggedInLocationName = loggedInLocationName.replace(/[\s_]*\d+$/, '');
            });
            ...
            var init = function () {
                if (!($scope.allTemplates !== undefined && $scope.allTemplates.length > 0)) {
                    spinner.forPromise(conceptSetService.getConcept({
                        ...
                        createConceptSections(allTemplates, loggedInLocationName);
            ...
            var initializeDefaultTemplates = function () {
                $scope.consultation.selectedObsTemplate = _.sortBy($scope.allTemplates, function (template) {
                    return template.options && template.options.order;
                });
            };
            ...
            var createConceptSections = function (allTemplates, loggedInLocationName) {
                _.map(allTemplates, function (template) {
                    var conceptSetExtension = _.find(extensions, function (extension) {
                        const defaultForms = extension.extensionParams;
                        return defaultForms.conceptName === template.name.name && (defaultForms.defaultLocation === loggedInLocationName || defaultForms.defaultLocation === "All");
                    }) || {};
