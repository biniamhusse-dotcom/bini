...
angular.module('opd.documentupload')
    .controller('DocumentController', ['$scope', '$stateParams', 'visitService', 'patientService', 'encounterService',
        ...
        function ($scope, $stateParams, visitService, patientService, encounterService, spinner, visitDocumentService,
            ...
            var getVisitTypeName = function (uuid) {
                var params = {
                    q: "emrapi.getVisitType",
                    uuid: uuid,
                }
            ...
            getVisitTypeName(locationUuid).then(function (response) {
                $rootScope.loginLocation = response.data[0].name;
            });

            var getRegistrationDate = function (uuid) {
                var params = {
                    q: "emrapi.getPersonRegistrationDate",
                    uuid: uuid,
                }
                return $http.get('/openmrs/ws/rest/v1/bahmnicore/sql', {
                    method: "GET",
                ...
                return visitService.search({
                    ...
                    if (visits.length > 0) {
                        ...
                        visitService.getVisitType().then(function (response) {
                            $scope.visitTypes = response.data.results;
                            $scope.visitTypes.forEach(vt => {
                                if ($rootScope.loginLocation === vt.display) {
                                    $scope.newVisit.visitType = vt;
                                }
                            })
                        });

                        // Fetch registration date
                        let registrationResponse = await getRegistrationDate($rootScope.patient.uuid);
                        $rootScope.patient.registrationDate = new Date(registrationResponse.data[0].registration_date).toISOString().split('T')[0];
                        $scope.newVisit.startDatetime = DateUtil.parse($rootScope.patient.registrationDate);
                        $scope.newVisit.stopDatetime = DateUtil.parse(today);
                    }
