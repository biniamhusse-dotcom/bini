...
angular.module('bahmni.ot')
    .controller('calendarViewController', ['$scope', '$rootScope', '$state', '$stateParams', '$window', 'appService', 'patientService', 'locationService', 'ngDialog','visitService', 'surgicalAppointmentHelper',
        function ($scope, $rootScope, $state, $stateParams, $window, appService, patientService, locationService, ngDialog, visitService, surgicalAppointmentHelper) {
            ...
            var getVisits = function(patientUuid) {
                return visitService.search({
                    patient: patientUuid, includeInactive: false, v: "custom:(uuid,location:(uuid))" }).then(function (response) {
                    return response.data.results;
                });
                
            };
        $scope.goToFollowUp = async function(event) {
            var patientUUID = $scope.surgicalBlockSelected.surgicalAppointments[0].patient.uuid;
            var url = '../clinical/index.html#/default/patient/' + patientUUID + '/otform/followup';
            var visits = await getVisits(patientUUID);

            if(visits.length>0){
                url = '../clinical/index.html#/default/patient/' + patientUUID + '/otform/followup';
            }
            else{
                url = '../clinical/index.html#/default/patient/' + patientUUID + '/dashboard';
            }
