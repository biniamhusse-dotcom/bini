                ...
                pre: function ($scope) {
                    ...
                    $scope.responseMap = function (data) {
                        return _.map(data, function (patientInfo) {
                            var middleName = patientInfo.middleName ? " " + patientInfo.middleName : '';
                            var familyName = patientInfo.familyName ? " " + patientInfo.familyName : '';
                            patientInfo.label = patientInfo.givenName + middleName + familyName + " " + "(" + patientInfo.identifier + ")";
                            return patientInfo;
                        });
                    };
                    ...
                    $scope.onSelectPatient = function (data) {
                        ...
                        spinner.forPromise(appointmentsService.search({patientUuid: data.uuid}).then(function (oldAppointments) {
                            var appointmentInDESCOrderBasedOnStartDateTime = _.sortBy(oldAppointments.data, "endDateTime").reverse();
