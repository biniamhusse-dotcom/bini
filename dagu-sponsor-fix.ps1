# ============================================================
# Dagu Sponsor & Mapping Fix Script (PowerShell 5.1+)
# Run this against the Dagu PostgreSQL database (eapts_dev)
# Usage: powershell -ExecutionPolicy Bypass -File dagu-sponsor-fix.ps1
# ============================================================
...
if (-not $env:PGDATABASE) { $env:PGDATABASE = "eapts_dev" }
if (-not $env:PGUSER) { $env:PGUSER = "postgres" }
if (-not $env:PGPASSWORD) { $env:PGPASSWORD = "6h5Q4W4gPC" }

$PSQL = "C:\Program Files\PostgreSQL\11\bin\psql.exe"

Write-Host "============================================"
Write-Host "  Dagu Sponsor & Mapping Fix"
Write-Host "  Target: $($env:PGDATABASE) @ $($env:PGHOST):$($env:PGPORT)"
Write-Host "============================================"
...
# --------------------------------------------------
Write-Host ""
Write-Host "[1/3] Setting all sponsors woreda_id = 5024 ..."
& $PSQL -h $env:PGHOST -p $env:PGPORT -U $env:PGUSER -d $env:PGDATABASE -c "UPDATE du.sponsor SET woreda_id = 5024 WHERE woreda_id IS DISTINCT FROM 5024;"
Write-Host "  Done. Verifying:"
& $PSQL -h $env:PGHOST -p $env:PGPORT -U $env:PGUSER -d $env:PGDATABASE -c "SELECT id, name, woreda_id FROM du.sponsor ORDER BY id;"

# --------------------------------------------------
# 2. Set OPD Pharmacy woreda_id = 5024 (Dire Dawa)
# --------------------------------------------------
Write-Host ""
Write-Host "[2/3] Setting OPD Pharmacy (12290) woreda_id = 5024 ..."
& $PSQL -h $env:PGHOST -p $env:PGPORT -U $env:PGUSER -d $env:PGDATABASE -c "UPDATE institution.institution SET woreda_id = 5024 WHERE id = 12290;"
Write-Host "  Done. Verifying all institutions:"
& $PSQL -h $env:PGHOST -p $env:PGPORT -U $env:PGUSER -d $env:PGDATABASE -c "SELECT id, name, woreda_id FROM institution.institution WHERE id IN (3101,12287,12290,12291,12293,2880670,2880671,2880672) ORDER BY id;"

# --------------------------------------------------
# 3. Populate sponsor_mapping (OpenMRS -> Dagu)
# --------------------------------------------------
Write-Host ""
Write-Host "[3/3] Populating common.sponsor_mapping ..."
...
& $PSQL -h $env:PGHOST -p $env:PGPORT -U $env:PGUSER -d $env:PGDATABASE -c $sql
Write-Host "  Done. Verifying:"
& $PSQL -h $env:PGHOST -p $env:PGPORT -U $env:PGUSER -d $env:PGDATABASE -c "SELECT count(*) as total_mappings FROM common.sponsor_mapping;"
& $PSQL -h $env:PGHOST -p $env:PGPORT -U $env:PGUSER -d $env:PGDATABASE -c "SELECT id, openmrs_concept_uuid, dagu_sponsor_id, description FROM common.sponsor_mapping ORDER BY id;"
