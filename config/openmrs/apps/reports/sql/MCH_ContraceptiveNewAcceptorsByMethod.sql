...
SELECT 'Oral contraceptives' AS "Method", 
    COALESCE(SUM(CASE WHEN ob.concept_id = 62733 AND ob.value_coded = 61785 THEN 1 ELSE 0 END), 0) AS Count
FROM obs ob
WHERE 
    EXISTS (
        SELECT 1 
        ...
        WHERE o.encounter_id = ob.encounter_id
            AND o.concept_id = 63919 AND o.value_coded = 62003
    )
...
SELECT 'Others' AS "Method", 
    COALESCE(SUM(CASE WHEN ob.concept_id = 62733 AND ob.value_coded = 57368 THEN 1 ELSE 0 END), 0) AS Count
FROM obs ob
WHERE 
    EXISTS (
        SELECT 1 
