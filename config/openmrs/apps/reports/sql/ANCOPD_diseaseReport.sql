...
FROM
    ...
    AND date(ob.date_created) BETWEEN '#startDate#'
    AND '#endDate#'
    JOIN concept c ON ob.value_coded = c.concept_id AND ob.voided = 0
    AND c.class_id = 4
...
WHERE
    EXISTS (
        ...
        WHERE
            ...
            AND o.value_coded = 67028 AND ob.voided = 0
    )
    AND EXISTS (
        SELECT
            1
        ...
        WHERE
            ...
            AND o.value_coded = 28 AND ob.voided = 0
    )
    AND ob.location_id IN(19, 20, 21, 22, 23, 24)
GROUP BY
    cn.name WITH ROLLUP;