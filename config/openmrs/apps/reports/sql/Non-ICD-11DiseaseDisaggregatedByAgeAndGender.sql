...
FROM (
    ...
    SELECT
        ...
        COALESCE(ob.value_text, 'Unknown Uncoded Diagnosis') AS Diagnosis,
    ...
    FROM person p
    JOIN obs ob ON p.person_id = ob.person_id AND ob.concept_id = 21 AND ob.voided = 0
    JOIN encounter e ON ob.encounter_id = e.encounter_id AND e.voided = 0
    JOIN location l ON e.location_id = l.location_id
    -- Link to the "Primary" (28) status via the same observation group
